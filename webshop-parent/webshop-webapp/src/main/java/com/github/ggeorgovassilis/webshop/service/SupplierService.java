...
public interface SupplierService {
	 ...
	 * 
	 * @param daysFromNow
	 *            Any positive number of days, 0 for current stock
	 * @return
	 */
	StockDTO getStock(int daysFromNow);

	/**
	 * Return status of herd at a future point in time. Assumes no consumption
	 * or replenishment of the herd.
	 * 
	 * @param daysFromNow
	 *            Any positive number of days.
	 * @return
	 */
	 ...
	 * 
	 * @param name
	 *            Name of animal to update. If the name does not exist yet,a new
	 *            animal is added to the herd
	 * @param age
	 * @param ageLastShorn
	 * @return
	 */
	...
	/**
	 * Places an order and returns the part of the order that could be
	 * satisfied. It returns the following http statuses: 201 - order fully
	 * placed 206 - order partially placed (over quota) 404 - order not placed
	 * 
	 * @param order
	 * @param daysFromNow
	 *            day on which to execute the order
	 * @return Will always return a {@link StockDTO} object with the quantities
	 *         that were actually served
	 */
	ResponseEntity<ReceiptDTO> placeOrder(@Valid OrderDTO order, int daysFromNow);
